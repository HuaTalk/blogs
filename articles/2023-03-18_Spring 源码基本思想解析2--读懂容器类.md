---
title: "Spring 源码基本思想解析2--读懂容器类"
date: 2023-03-18
url: https://juejin.cn/post/7211734661764251685
views: 120
likes: 1
collects: 0
source: html2md
...
函数式编程思想下，一个对象通常值为不变的。这样做的好处有很多，仅列举一些：便于支持并发，是线程安全的；对象是无状态的，极大地简化代码的理解，因为每一次值的变化都是显性的；便于编译器优化性能，比如延迟计算，实现对象复用；便于测试，因为函数没有副作用，每一次执行方法返回的结果都一样。
...
也可以用IDEA提供的Structure功能看，一目了然。 ![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4c892dfb649d46b4913b69c1b34d9dce~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)

打断点到对应位置 ![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/825a0aa445e444a2994746555d04e441~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)

模板方法refresh ![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8c5022a956ee4a76a479b0872248bbc6~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
使用@Order 注解可以为一组类进行排序，再阅读源码之前，我们虽然不知道是哪个具体的类去处理的，但是其处理的过程必然包括了以下过程：获取注解值，对原有类与排序号进行关联，一个通用的排序方法排序。
...
## 总结
  ...
  3. 关注数据的传递，可以说数据的传递表达了系统的设计架构。有时候虽然我们并不知道数据的传递过程，但是我们更在意数据传递的结果。